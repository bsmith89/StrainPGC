...
We benchmarked resource utilization on our servers using the quick-start example described above.
...
- Disk (Between 160 and 200 GB)
    ...
    - The portion of the UHGG MIDASDB used here (71 species profiled downloaded as one step in the integrated workflow for the ) is > 20 GB.
    ...
    - Intermediate files generated by the workflow totaled > 70 GB. Some of
      ...
      their purpose, as they're marked with `temp(...)` in the Snakefile.
      However, the provided Snakemake profiles all include the `--notemp` flag.
    - Downloaded apptainer containers total > 5 GB
- RAM (~90 GB at peak)
...
- CPU Time
    - In particular, building the Bowtie2 index takes a long time:
      about ~60 hours of CPU time partially parallelized over 48 processes for a total walltime of ~130 minutes.
    - Running MIDAS pangenome profiling for the largest sample (~13 million reads) took ~3.5 hours of CPU time.
      This was efficiently split across four processes for a total of ~45 minutes of walltime.
